...
import {
  StationAndAvailabilityCityNameZhTW,
  StationAndAvailabilityCityName,
  CyclingShapeCityName,
  CyclingShapeCityNameZhTW,
} from "@/types";
...
function updateCurrentByUID(UID: string) {
  ...
  let desInfo = bikeStore.bikeStations.find(
    (station) => station.StationUID === UID
  );
...
function getStationData() {
  if (selectCity.value && lat && lon)
    bikeStore.getStationData(selectCity.value, lat, lon);
}
...
function switchAvailability(state: "rent" | "return") {
  switch (state) {
    case "rent": //借車
      ...
      updateCurrentByUID(bikeStore.getSelectedBikeStation.StationUID); //測試用篩選號碼
      break;
    case "return": //還車
      isRent.value = false;
...
<template>
  <main class="content">
    <!-- <h1>{{ msg }}</h1> -->
    <article
      v-if="bikeStore.userLocation[0] && bikeStore.userLocation[1]"
      style="position: absolute; top: 0; left: 0; height: 100%; width: 100%"
    >
      <Map
        :location="[bikeStore.userLocation[0], bikeStore.userLocation[1]]"
        :stations="bikeStore.getNearByStations"
        :availability="bikeStore.getNearByAvailability"
        :isRent="bikeStore.getIsRent"
      />
    </article>
    ...
    <article class="car-nav">
      ...
      <div class="Homecar">
        <div class="car-btns">
          <button
            type="button"
            class="btn-orange se_btn"
            @click="switchAvailability('rent')"
          >
            借車
          </button>
          <button
            type="button"
            class="disable_g btn-green se_btn"
            @click="switchAvailability('return')"
          >
            還車
          </button>
        </div>
    ...
    <article class="Homeimg">
      ...
      <p>
        趕快開始騎上UBike四處旅遊吧 ! 邊騎腳踏車邊用手機是很危險的喔 (´･ω･`)
      </p>
    </article>
  </main>
  ...
  <article class="HomeCurrentStation" v-if="bikeStore.getSelectedBikeStation.name!=null">
    ...
    <div>
      可借:
      <span class="number">{{
        bikeStore.getSelectedBikeStation?.availableRent|| 0
      }}</span>
      可還:<span class="number">{{
        bikeStore.getSelectedBikeStation?.availableReturn|| 0
      }}</span>
    </div>
  </article>
  <div @click="handleGPS()">
